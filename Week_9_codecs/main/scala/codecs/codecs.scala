...
sealed trait Json {
  ...
  /**
    ...
    * }}}
    */
  def decodeAs[A](implicit decoder: Decoder[A]): Option[A] =
    decoder.decode(this)
}
...
object Decoder extends DecoderInstances {
  ...
  /**
    ...
    */
  def fromFunction[A](f: Json => Option[A]): Decoder[A] =
    new Decoder[A] {
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(x) if (x.isValidInt) => Some(x.toInt)
      case _                             => Option.empty
    }
  ...
  implicit val stringDecoder: Decoder[String] =
    Decoder.fromFunction {
      case Json.Str(x) => Some(x.toString)
      case _           => Option.empty
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder
      ...
      .transform[Person]({ case (x, y) => Person(x, y) })
