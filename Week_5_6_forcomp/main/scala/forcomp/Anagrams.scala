...
object Anagrams extends AnagramsInterface:
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (for ((ch, list) <- (w.toLowerCase groupBy (ch => ch))) yield ch -> list.size).toList.sorted
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap)((acc, b) => {
    val occ = acc(b._1) - b._2
    if (occ == 0) acc - b._1
    else acc.updated(b._1, occ)
  }).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        result <- sentenceAnagrams(subtract(occurrences, combination))
      } yield word :: result
    }
